...
static int zebra_read_route(ZAPI_CALLBACK_ARGS)
{
	...
	if (bgp_debug_zebra(&api.prefix)) {
		...
		if (add) {
			...
			zlog_debug("Rx route ADD VRF %u %s[%d] %pFX nexthop %s (type %d if %u) metric %u distance %u tag %" ROUTE_TAG_PRI,
				   vrf_id, zebra_route_string(api.type),
				   api.instance, &api.prefix, buf, nhtype,
				   ifindex, api.metric, api.distance, api.tag);
		} else {
	...
	if (local->sa.sa_family == AF_INET6) {
		...
		if (!IN6_IS_ADDR_LINKLOCAL(&local->sin6.sin6_addr)) {
			...
			if (!v6_ll_avail && if_is_loopback(ifp))
				...
				flog_warn(EC_BGP_NO_LL_ADDRESS_AVAILABLE,
		...
		/* Link-local address. */
		{
			...
			if (!ret)
				memcpy(&nexthop->v6_global,
				       &local->sin6.sin6_addr, IPV6_MAX_BYTELEN);
...
static struct in6_addr *bgp_path_info_to_ipv6_nexthop(struct bgp_path_info *path,
						      ifindex_t *ifindex)
{
				...
				struct bgp_path_info *path)
{
	...
	if (bgp_debug_zebra(p)) {
		if (p->family == AF_INET) {
			zlog_debug("Zebra rmap deny: IPv4 route %pFX nexthop %pI4",
				   p, &path->attr->nexthop);
		}
		if (p->family == AF_INET6) {
			ifindex_t ifindex;
			...
			zlog_debug("Zebra rmap deny: IPv6 route %pFX nexthop %pI6",
				   p, nexthop);
		}
	}
	return false;
	...
	if (ret < 0) {
		...
		if (!bgp_tm_chunk_obtained) {
			if (bgp_zebra_get_table_range(bgp_tm_chunk_size,
				...
				bgp_zebra_announce_table(bgp_tm_bgp, AFI_IP,
							 SAFI_FLOWSPEC);
			}
		}
	}
...
int bgp_zebra_get_table_range(uint32_t chunk_size, uint32_t *start,
			      uint32_t *end)
{
	int ret;
		...
		*mpinfo_cp = *mpinfo;
		...
		if (bgp_debug_zebra(&api.prefix)) {
			if (mpinfo->extra) {
				...
				zlog_debug("%s: p=%pFX, extra is NULL, no label",
					   __func__, p);
			}
		}
		...
		if (nh_family == AF_INET) {
			nh_updated =
				update_ipv4nh_for_route_install(nh_othervrf,
								bgp_orig,
								&mpinfo_cp->attr
									 ->nexthop,
								mpinfo_cp->attr,
			...
			if (!nexthop)
				...
				nh_updated =
					update_ipv6nh_for_route_install(nh_othervrf,
									bgp_orig,
									nexthop,
									ifindex,
									mpinfo,
		...
		if (labels && (num_labels > 0) &&
			...
			if (is_evpn) {
				nh_label =
					*bgp_evpn_path_info_labels_get_l3vni(labels,
									     num_labels);
	...
	if (bgp_debug_zebra(p)) {
		...
		for (i = 0; i < api.nexthop_num; i++) {
			...
			zlog_debug("  nhop [%d]: %s if %u VRF %u wt %u %s %s %s",
				   ...
				   api_nh->vrf_id, api_nh->weight, label_buf,
				   segs_buf, eth_buf);
		}
...
void bgp_zebra_withdraw_table_all_subtypes(struct bgp *bgp, afi_t afi,
					   safi_t safi)
{
	struct bgp_dest *dest;
	struct bgp_table *table;
			 ...
			 unsigned short instance, bool changed)
{
	...
	if (instance) {
		...
		if (EVPN_ENABLED(bgp) && type == ZEBRA_ROUTE_VNC_DIRECT) {
			vnc_export_bgp_enable(bgp, afi); /* only enables if mode
							    bits cfg'd */
		}
#endif
...
static int bgp_zebra_process_local_l3vni(ZAPI_CALLBACK_ARGS)
{
	...
	if (cmd == ZEBRA_L3VNI_ADD) {
		...
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug("Rx L3-VNI ADD VRF %s VNI %u Originator-IP %pI4 RMAC svi-mac %pEA vrr-mac %pEA filter %s svi-if %u",
...
static int bgp_zebra_process_local_macip(ZAPI_CALLBACK_ARGS)
{
	...
	if (BGP_DEBUG(zebra, ZEBRA))
		zlog_debug("%u:Recv MACIP %s f 0x%x MAC %pEA IP %pIA VNI %u seq %u state %d ESI %s",
			   vrf_id, (cmd == ZEBRA_MACIP_ADD) ? "Add" : "Del",
			   flags, &mac, &ip, vni, seqnum, state,
...
void bgp_send_pbr_iptable(struct bgp_pbr_action *pba, struct bgp_pbr_match *pbm,
			  bool install)
{
	struct stream *s;
	int ret = 0;
	...
	zclient_create_header(s,
			      install ? ZEBRA_IPTABLE_ADD : ZEBRA_IPTABLE_DELETE,
			      VRF_DEFAULT);
 ...
 */
void bgp_zebra_announce_default(struct bgp *bgp, struct nexthop *nh, afi_t afi,
...
int bgp_zebra_send_capabilities(struct bgp *bgp, bool disable)
{
	...
	if (zclient_capabilities_send(ZEBRA_CLIENT_CAPABILITIES, zclient,
				      &api) == ZCLIENT_SEND_FAILURE) {
...
void bgp_zebra_send_nexthop_label(int cmd, mpls_label_t label,
				  ...
				  uint32_t num_labels, mpls_label_t out_labels[])
{
