...
struct bpacket *subgroup_update_packet(struct update_subgroup *subgrp)
{
	...
	while (adv) {
		...
		if (stream_empty(s)) {
			 ...
			 * attr. */
			total_attr_len =
				bgp_packet_attribute(NULL, peer, s,
						     adv->baa->attr, &vecarr,
						     NULL, afi, safi, from,
			...
			if (space_remaining < space_needed) {
				...
				while (adv)
					adv = bgp_advertise_clean_subgroup(subgrp,
									   adj);
				return NULL;
			}
		...
		if ((afi == AFI_IP && safi == SAFI_UNICAST) &&
			...
			if (safi == SAFI_LABELED_UNICAST) {
				...
				num_labels = 1;
				   ...
				   bgp_mplsvpn_path_uses_valid_mpls_label(path) &&
			...
			if (stream_empty(snlri))
				mpattrlen_pos =
					bgp_packet_mpattr_start(snlri, peer, afi,
								safi, &vecarr,
								adv->baa->attr);
