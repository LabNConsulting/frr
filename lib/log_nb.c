...
static int logging_facility_modify(struct nb_cb_modify_args *args)
{
	...
	zlog_syslog_set_facility(facility_match(yang_dnode_get_string(args->dnode, NULL)));
...
const struct frr_yang_module_info frr_logging_nb_info = {
	...
	.nodes = {
		...
		},
		{
			...
			.cbs = {
				.create = logging_extended_syslog_create,
				.destroy = logging_extended_syslog_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/file",
			.cbs = {
				.create = logging_extended_syslog_destination_file_create,
				.destroy = logging_extended_syslog_destination_file_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/file/path",
			.cbs = {
				.modify = logging_extended_syslog_destination_file_path_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/file/user",
			.cbs = {
				.modify = logging_extended_syslog_destination_file_user_modify,
				.destroy = logging_extended_syslog_destination_file_user_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/file/group",
			.cbs = {
				.modify = logging_extended_syslog_destination_file_group_modify,
				.destroy = logging_extended_syslog_destination_file_group_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/file/mode",
			.cbs = {
				.modify = logging_extended_syslog_destination_file_mode_modify,
				.destroy = logging_extended_syslog_destination_file_mode_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/file/no-create",
			.cbs = {
				.modify = logging_extended_syslog_destination_file_no_create_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/file/format",
			.cbs = {
				.modify = logging_extended_syslog_destination_file_format_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fifo",
			.cbs = {
				.create = logging_extended_syslog_destination_fifo_create,
				.destroy = logging_extended_syslog_destination_fifo_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fifo/path",
			.cbs = {
				.modify = logging_extended_syslog_destination_fifo_path_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fifo/user",
			.cbs = {
				.modify = logging_extended_syslog_destination_fifo_user_modify,
				.destroy = logging_extended_syslog_destination_fifo_user_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fifo/group",
			.cbs = {
				.modify = logging_extended_syslog_destination_fifo_group_modify,
				.destroy = logging_extended_syslog_destination_fifo_group_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fifo/mode",
			.cbs = {
				.modify = logging_extended_syslog_destination_fifo_mode_modify,
				.destroy = logging_extended_syslog_destination_fifo_mode_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fifo/no-create",
			.cbs = {
				.modify = logging_extended_syslog_destination_fifo_no_create_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fifo/format",
			.cbs = {
				.modify = logging_extended_syslog_destination_fifo_format_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/unix",
			.cbs = {
				.create = logging_extended_syslog_destination_unix_create,
				.destroy = logging_extended_syslog_destination_unix_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/unix/path",
			.cbs = {
				.modify = logging_extended_syslog_destination_unix_path_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/unix/format",
			.cbs = {
				.modify = logging_extended_syslog_destination_unix_format_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/journald",
			.cbs = {
				.create = logging_extended_syslog_destination_journald_create,
				.destroy = logging_extended_syslog_destination_journald_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/syslog",
			.cbs = {
				.create = logging_extended_syslog_destination_syslog_create,
				.destroy = logging_extended_syslog_destination_syslog_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/syslog/supports-rfc5424",
			.cbs = {
				.modify = logging_extended_syslog_destination_syslog_supports_rfc5424_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fd",
			.cbs = {
				.create = logging_extended_syslog_destination_fd_create,
				.destroy = logging_extended_syslog_destination_fd_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fd/number",
			.cbs = {
				.modify = logging_extended_syslog_destination_fd_number_modify,
				.destroy = logging_extended_syslog_destination_fd_number_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fd/envvar",
			.cbs = {
				.modify = logging_extended_syslog_destination_fd_envvar_modify,
				.destroy = logging_extended_syslog_destination_fd_envvar_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fd/stdout",
			.cbs = {
				.create = logging_extended_syslog_destination_fd_stdout_create,
				.destroy = logging_extended_syslog_destination_fd_stdout_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fd/stderr",
			.cbs = {
				.create = logging_extended_syslog_destination_fd_stderr_create,
				.destroy = logging_extended_syslog_destination_fd_stderr_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/fd/format",
			.cbs = {
				.modify = logging_extended_syslog_destination_fd_format_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/destination/none",
			.cbs = {
				.create = logging_extended_syslog_destination_none_create,
				.destroy = logging_extended_syslog_destination_none_destroy,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/level",
			.cbs = {
				.modify = logging_extended_syslog_level_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/facility",
			.cbs = {
				.modify = logging_extended_syslog_facility_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/code-location",
			.cbs = {
				.modify = logging_extended_syslog_code_location_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/version",
			.cbs = {
				.modify = logging_extended_syslog_version_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/error-category",
			.cbs = {
				.modify = logging_extended_syslog_error_category_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/unique-id",
			.cbs = {
				.modify = logging_extended_syslog_unique_id_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/format-args",
			.cbs = {
				.modify = logging_extended_syslog_format_args_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/timestamp-precision",
			.cbs = {
				.modify = logging_extended_syslog_timestamp_precision_modify,
			}
		},
		{
			.xpath = "/frr-logging:logging/extended-syslog/timestamp-local-time",
			.cbs = {
				.modify = logging_extended_syslog_timestamp_local_time_modify,
			}
		},
		{
			.xpath = NULL,
