...
static void vty_out_cpu_event_history(struct vty *vty, struct cpu_event_history *a)
{
...
struct event_loop *event_master_create(const char *name)
{
	...
	rv->read = XCALLOC(MTYPE_EVENT_POLL, sizeof(struct event *) * rv->fd_limit);
	...
	snprintf(tmhashname, sizeof(tmhashname), "%s - threadmaster event hash",
		 name);
	rv->cpu_record =
		hash_create_size(8, (unsigned int (*)(const void *))cpu_record_hash_key,
				 (bool (*)(const void *,
 ...
 */
static void cancel_arg_helper(struct event_loop *master, const struct cancel_req *cr)
{
	struct event *t;
	nfds_t i;
	int fd;
...
void event_call(struct event *thread)
{
	...
	if (cputime_enabled_here && cputime_enabled) {
		...
		exp = atomic_load_explicit(&thread->hist->cpu.max,
					   memory_order_seq_cst);
		while (exp < cputime &&
		       !atomic_compare_exchange_weak_explicit(&thread->hist->cpu.max,
							      &exp, cputime,
							      memory_order_seq_cst,
							      memory_order_seq_cst))
			;
