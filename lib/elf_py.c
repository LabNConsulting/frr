...
#define member(name, type, doc)                                                \
	{                                                                      \
		(char *)#name, type, offsetof(struct elfreloc, name),          \
			READONLY,                                              \
			(char *)doc "\n\n(\"" #name "\", " #type               \
				    " in elf_py.c)"                            \
	}
				    ...
				    GElf_Shdr *relhdr)
{
	...
	for (i = 0; i < entries; i++) {
		...
		relw = (struct elfreloc *)
			       typeobj_elfreloc.tp_alloc(&typeobj_elfreloc, 0);
		relw->es = w;
...
static PyObject *elffile_find_note(PyObject *self, PyObject *args)
{
	...
	for (i = 0; i < w->ehdr->e_phnum; i++) {
		...
		while ((offset = gelf_getnote(notedata, offset, nhdr, &nameoffs,
			...
			s = PyLong_FromUnsignedLongLong(phdr->p_vaddr +
							dataoffs);
...
static PyObject *elffile_load(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
	...
	for (size_t i = 0; i < w->ehdr->e_phnum; i++) {
		...
		Elf_Data *dyndata =
			elf_getdata_rawchunk(w->elf, phdr->p_offset,
...
out_elferr:
	...
	PyErr_Format(ELFFormatError, "libelf error %d: %s", err,
		     elf_errmsg(err));
out:
	if (w->elf)
		elf_end(w->elf);
...
static PyMethodDef methods_elfpy[] = {{"elfpy_debug", elfpy_debug, METH_VARARGS,
