...
#define frr_each_safe(prefix, head, item)                                      \
	for (typeof(prefix##_next_safe(head, NULL)) prefix##_safe =            \
	     ...
	     item;                                                             \
	     item = prefix##_safe, prefix##_safe =                             \
...
#define DECLARE_LIST(prefix, type, field)                                         \
	...
	macro_pure const type *prefix##_const_first(                              \
		const struct prefix##_head *h)                                    \
	{                                                                         \
		if (h->sh.first != _SLIST_LAST)                                   \
	...
	macro_pure type *prefix##_next_safe(struct prefix##_head *h,              \
					    type *item)                           \
	{                                                                         \
...
#define DECLARE_DLIST(prefix, type, field)                                        \
	...
	TYPESAFE_FIRST_NEXT(prefix, type)                                         \
	macro_pure const type *prefix##_const_last(                               \
		const struct prefix##_head *h)                                    \
	{                                                                         \
	...
	macro_pure type *prefix##_prev_safe(struct prefix##_head *h,              \
					    type *item)                           \
	{                                                                         \
		if (!item)                                                        \
			return NULL;                                              \
...
#define DECLARE_HEAP(prefix, type, field, cmpfn)                                  \
	...
	{                                                                         \
		...
		if (cmpfn(container_of(other, type, field.hi), item) < 0)         \
			typesafe_heap_pullup(&h->hh, index, other,                \
					     prefix##__cmp);                      \
		else                                                              \
			typesafe_heap_pushdown(&h->hh, index, other,              \
					       prefix##__cmp);                    \
...
#define _DECLARE_SORTLIST(prefix, type, field, cmpfn_nuq, cmpfn_uq)               \
	...
	{                                                                         \
		...
		int c = 1;                                                        \
		while (*np && (c = cmpfn_uq(container_of(*np, type, field.si),    \
					    item)) < 0)                           \
			np = &(*np)->next;                                        \
		if (c == 0)                                                       \
	...
	macro_inline const type                                                   \
		*prefix##_const_find_gteq(const struct prefix##_head *h,          \
					  const type *item)                       \
	{                                                                         \
		const struct ssort_item *sitem = h->sh.first;                     \
		int cmpval = 0;                                                   \
		while (sitem &&                                                   \
		       (cmpval = cmpfn_nuq(container_of(sitem, type, field.si),   \
					   item)) < 0)                            \
			sitem = sitem->next;                                      \
		return container_of_null(sitem, type, field.si);                  \
	}                                                                         \
	macro_inline const type                                                   \
		*prefix##_const_find_lt(const struct prefix##_head *h,            \
					const type *item)                         \
	{                                                                         \
...
#define DECLARE_SORTLIST_UNIQ(prefix, type, field, cmpfn)                      \
	_DECLARE_SORTLIST(prefix, type, field, cmpfn, cmpfn);                  \
                                                                               \
	macro_inline const type                                                \
		*prefix##_const_find(const struct prefix##_head *h,            \
				     const type *item)                         \
	{                                                                      \
		const struct ssort_item *sitem = h->sh.first;                  \
		int cmpval = 0;                                                \
		while (sitem &&                                                \
		       (cmpval = cmpfn(container_of(sitem, type, field.si),    \
				       item)) < 0)                             \
			sitem = sitem->next;                                   \
		if (!sitem || cmpval > 0)                                      \
			return NULL;                                           \
...
#define HASH_SHRINK_THRESHOLD(head) ((head).count <= (HASH_SIZE(head) - 1) / 2)
...
#define DECLARE_HASH(prefix, type, field, cmpfn, hashfn)                          \
	...
	{                                                                         \
		...
		while (*np && (*np)->hashval < hval)                              \
			np = &(*np)->next;                                        \
		while (*np && (*np)->hashval == hval) {                           \
			if (cmpfn(container_of(*np, type, field.hi), item) ==     \
			    0) {                                                  \
				h->hh.count--;                                    \
	...
	{                                                                         \
		if (!h->hh.tabshift)                                              \
			return NULL;                                              \
		uint32_t hval = item->field.hi.hashval,                           \
			 hbits = HASH_KEY(h->hh, hval);                           \
		struct thash_item **np = &h->hh.entries[hbits];                   \
		while (*np && (*np)->hashval < hval)                              \
			np = &(*np)->next;                                        \
	...
	{                                                                         \
		...
		for (i = 0; i < HASH_SIZE(h->hh); i++)                            \
			if (h->hh.entries[i])                                     \
				return container_of(h->hh.entries[i], type,       \
						    field.hi);                    \
		return NULL;                                                      \
	}                                                                         \
	macro_pure const type *prefix##_const_next(const struct prefix##_head *h, \
	...
	{                                                                         \
		...
		while (hitem && hitem->hashval < hval)                            \
			hitem = hitem->next;                                      \
		for (hitem = h->hh.entries[hbits];                                \
		     hitem && hitem->hashval <= hval; hitem = hitem->next)        \
...
#define _DECLARE_SKIPLIST(prefix, type, field, cmpfn_nuq, cmpfn_uq)               \
	...
	{                                                                         \
		const struct sskip_item *sitem =                                  \
			typesafe_skiplist_find_gteq(&h->sh, &item->field.si,      \
						    cmpfn_nuq);                   \
		return container_of_null(sitem, type, field.si);                  \
	}                                                                         \
	macro_inline const type                                                   \
	...
	{                                                                         \
		const struct sskip_item *sitem =                                  \
			typesafe_skiplist_find_lt(&h->sh, &item->field.si,        \
						  cmpfn_nuq);                     \
		return container_of_null(sitem, type, field.si);                  \
	}                                                                         \
	TYPESAFE_FIND_CMP(prefix, type)                                           \
	...
	{                                                                         \
		struct sskip_item *sitem =                                        \
			typesafe_skiplist_del(&h->sh, &item->field.si,            \
					      cmpfn_uq);                          \
		return container_of_null(sitem, type, field.si);                  \
	}                                                                         \
	macro_inline type *prefix##_pop(struct prefix##_head *h)                  \
...
#define DECLARE_SKIPLIST_UNIQ(prefix, type, field, cmpfn)                      \
	...
	{                                                                      \
		const struct sskip_item *sitem =                               \
			typesafe_skiplist_find(&h->sh, &item->field.si,        \
					       &prefix##__cmp);                \
		return container_of_null(sitem, type, field.si);               \
	}                                                                      \
	TYPESAFE_FIND(prefix, type)                                            \
...
#define DECLARE_SKIPLIST_NONUNIQ(prefix, type, field, cmpfn)                   \
	...
	{                                                                      \
		...
		return 0;                                                      \
                                                                               ...
                                                                               \
	_DECLARE_SKIPLIST(prefix, type, field, prefix##__cmp,                  \
			  prefix##__cmp_uq);                                   \
	TYPESAFE_MEMBER_VIA_FIND_GTEQ(prefix, type, cmpfn)                     \
	MACRO_REQUIRE_SEMICOLON() /* end */
