...
static void vrf_update_state(struct vrf *vrf)
{
	...
	nb_op_updatef(vrf->state, "state/id", "%d", vrf->vrf_id);
	nb_op_update(vrf->state, "state/active",
		     CHECK_FLAG(vrf->status, VRF_ACTIVE) ? "true" : "false");
}
...
struct vrf *vrf_get(vrf_id_t vrf_id, const char *name)
{
	...
	if (name && vrf->name[0] != '\0' && strcmp(name, vrf->name)) {
		...
		/* XXX chopps we're deleting cache state, but not notifying augmentation */
		if (vrf_notify_oper_changes)
			nb_op_update_delete(vrf->state, NULL);
		...
		if (vrf_notify_oper_changes)
			vrf->state = nb_op_update_pathf(NULL, "/frr-vrf:lib/vrf[name=\"%s\"]", NULL,
							vrf->name);
