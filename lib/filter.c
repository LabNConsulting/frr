...
enum filter_type access_list_apply(struct access_list *access, const void *object)
{
	struct filter *filter;
...
static void config_write_access_zebra(struct vty *, struct filter *, json_object *);
...
static int filter_show(struct vty *vty, const char *name, afi_t afi, bool use_json)
{
	struct access_list *access;
	struct access_master *master;
...
static const struct cmd_variable_handler access_list_handlers[] = {
	...
	{ .tokenname = "ACCESSLIST4_NAME", .completions = access_list4_autocomplete },
	{ .tokenname = "ACCESSLIST6_NAME", .completions = access_list6_autocomplete },
