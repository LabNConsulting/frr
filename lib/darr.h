...
/**
 * darr_len() - Get the current length of the array as size_t
 * darr_ilen() - Get the current length of the array as ssize_t
 *
...
#define darr_in_strdup_cap(D, S, C)                                            \
	({                                                                     \
		ssize_t __size = strlen(S) + 1;                                \
		darr_reset(D);                                                 \
		darr_ensure_cap(D, ((ssize_t)(C) > __size) ? (ssize_t)(C)      \
							   : __size);          \
		strlcpy(D, (S), darr_cap(D));                                  \
		darr_setlen((D), (size_t)__size);                              \
		D;                                                             \
	})
#define darr_in_strdup(D, S) darr_in_strdup_cap(D, S, 0)
...
#define darr_strdup_cap(S, C)                                                  \
	({                                                                     \
		ssize_t __size = strlen(S) + 1;                                \
		char *__s = NULL;                                              \
		darr_ensure_cap(__s, ((ssize_t)(C) > __size) ? (ssize_t)(C)    \
							     : __size);        \
