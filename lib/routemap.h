...
typedef enum { RMAP_DENYMATCH, RMAP_PERMITMATCH } route_map_result_t;
...
#define IS_MATCH_MAC_LIST(C)	(strmatch(C, "frr-bgp-route-map:mac-address-list"))
...
#define IS_MATCH_ROUTE_SRC(C) (strmatch(C, "frr-bgp-route-map:ip-route-source"))
#define IS_MATCH_ROUTE_SRC_PL(C)                                               \
	(strmatch(C, "frr-bgp-route-map:ip-route-source-prefix-list"))
#define IS_MATCH_COMMUNITY(C) (strmatch(C, "frr-bgp-route-map:match-community"))
...
#define IS_SET_ORIGINATOR_ID(A) (strmatch(A, "frr-bgp-route-map:originator-id"))
...
#define IS_SET_AS_PREPEND(A) (strmatch(A, "frr-bgp-route-map:as-path-prepend"))
#define IS_SET_AS_EXCLUDE(A) (strmatch(A, "frr-bgp-route-map:as-path-exclude"))
...
extern enum rmap_compile_rets route_map_delete_set(struct route_map_index *index,
						   const char *set_name,
						   const char *set_arg);
...
extern void route_map_notify_pentry_dependencies(const char *affected_name,
						 struct prefix_list_entry *pentry,
						 route_map_event_t event);
extern int generic_match_add(struct route_map_index *index, const char *command,
			     const char *arg, route_map_event_t type,
			     char *errmsg, size_t errmsg_len);
extern int generic_match_delete(struct route_map_index *index, const char *command,
				const char *arg, route_map_event_t type,
				char *errmsg, size_t errmsg_len);

extern int generic_set_add(struct route_map_index *index, const char *command,
			   const char *arg, char *errmsg, size_t errmsg_len);
extern int generic_set_delete(struct route_map_index *index, const char *command,
			      const char *arg, char *errmsg, size_t errmsg_len);
...
struct route_map_match_set_hooks {
	...
	/* match ip address */
	int (*match_ip_address)(struct route_map_index *index, const char *command,
				const char *arg, route_map_event_t type,
				char *errmsg, size_t errmsg_len);
	...
	/* match ip next hop */
	int (*match_ip_next_hop)(struct route_map_index *index, const char *command,
				 const char *arg, route_map_event_t type,
				 char *errmsg, size_t errmsg_len);
...
typedef int (*routemap_match_hook_fun)(struct route_map_index *rmi,
				       const char *command, const char *arg,
				       route_map_event_t event, char *errmsg,
				       size_t errmsg_len);
...
extern void route_map_instance_show(struct vty *vty, const struct lyd_node *dnode,
				    bool show_defaults);
extern void route_map_instance_show_end(struct vty *vty,
					const struct lyd_node *dnode);
extern void route_map_condition_show(struct vty *vty, const struct lyd_node *dnode,
				     bool show_defaults);
