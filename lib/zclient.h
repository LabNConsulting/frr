...
static inline const char *zapi_nhg_notify_owner2str(enum zapi_nhg_notify_owner note)
{
	const char *ret = "UNKNOWN";
...
static inline const char *zapi_rule_notify_owner2str(enum zapi_rule_notify_owner note)
{
	const char *ret = "UNKNOWN";
...
extern unsigned short *redist_check_instance(struct redist_proto *, unsigned short);
...
extern enum zclient_send_status
zclient_send_router_id_update(struct zclient *zclient, zebra_message_types_t type,
...
extern int zapi_srv6_locator_encode(struct stream *s, const struct srv6_locator *l);
...
extern enum zclient_send_status zclient_nhg_send(struct zclient *zclient, int cmd,
...
extern enum zclient_send_status zclient_capabilities_send(uint32_t cmd,
...
extern enum zclient_send_status zclient_send_mlag_register(struct zclient *client,
							   uint32_t bit_map);
extern enum zclient_send_status zclient_send_mlag_deregister(struct zclient *client);
...
enum zclient_send_status zclient_unregister_opaque(struct zclient *zclient,
						   uint32_t type);
int zapi_opaque_reg_decode(struct stream *msg, struct zapi_opaque_reg_info *info);
