...
static inline void rbe_set_blackred(struct rb_entry *black, struct rb_entry *red)
{
...
static inline void rbe_remove_color(const struct rb_type *t, struct rbt_tree *rbt,
...
void *_rb_find(const struct rb_type *t, const struct rbt_tree *rbt, const void *key)
{
...
void *_rb_nfind(const struct rb_type *t, const struct rbt_tree *rbt, const void *key)
{
	struct rb_entry *tmp = RBH_ROOT(rbt);
	void *node;
	void *res = NULL;
...
void _rb_poison(const struct rb_type *t, void *node, unsigned long poison)
{
	...
	RBE_PARENT(rbe) = RBE_LEFT(rbe) = RBE_RIGHT(rbe) = (struct rb_entry *)poison;
}
