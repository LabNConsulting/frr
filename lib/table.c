...
struct route_node *route_node_get(struct route_table *table, union prefixconstptr pu)
{
 ...
 * Default delegate.
 */
static route_table_delegate_t default_delegate = { .create_node = route_node_create,
						   .destroy_node =
							   route_node_destroy };
 ...
 */
static struct route_node *route_table_get_next_internal(struct route_table *table,
							const struct prefix *p)
{
	...
	while (node) {
		...
		if (match) {
			 ...
			 */
			tmp_node =
				node->link[prefix_bit(&p->u.prefix, node->p.prefixlen)];
