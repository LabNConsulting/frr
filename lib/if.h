...
#define IFNAME_RB_INSERT(v, ifp)                                                       \
	({                                                                             \
		struct interface *_iz = RB_INSERT(if_name_head,                        \
						  &v->ifaces_by_name, (ifp));          \
		if (_iz)                                                               \
...
#define IFNAME_RB_REMOVE(v, ifp)                                                       \
	({                                                                             \
		struct interface *_iz = RB_REMOVE(if_name_head,                        \
						  &v->ifaces_by_name, (ifp));          \
		if (_iz == NULL)                                                       \
			flog_err(EC_LIB_INTERFACE,                                     \
				 "%s(%s): corruption detected -- interface with this " \
...
#define IFINDEX_RB_INSERT(v, ifp)                                                      \
	({                                                                             \
		struct interface *_iz = RB_INSERT(if_index_head,                       \
						  &v->ifaces_by_index, (ifp));         \
		if (_iz)                                                               \
			flog_err(EC_LIB_INTERFACE,                                     \
...
#define IFINDEX_RB_REMOVE(v, ifp)                                                      \
	({                                                                             \
		struct interface *_iz = RB_REMOVE(if_index_head,                       \
						  &v->ifaces_by_index, (ifp));         \
		if (_iz == NULL)                                                       \
			flog_err(EC_LIB_INTERFACE,                                     \
				 "%s(%u): corruption detected -- interface with this " \
