...
static struct mgmt_be_batch_ctx *mgmt_be_batch_create(struct mgmt_be_txn_ctx *txn,
						      uint64_t batch_id)
{
	struct mgmt_be_batch_ctx *batch = NULL;
...
static struct mgmt_be_txn_ctx *mgmt_be_find_txn_by_id(struct mgmt_be_client *client_ctx,
						      uint64_t txn_id, bool warn)
{
...
static struct mgmt_be_txn_ctx *mgmt_be_txn_create(struct mgmt_be_client *client_ctx,
						  uint64_t txn_id)
{
	struct mgmt_be_txn_ctx *txn = NULL;
				    ...
				    const char *error_if_any)
{
	...
	MGMTD_BE_CLIENT_DBG("Sending CFG_APPLY_REPLY txn-id %" PRIu64
			    ...
			    success && num_batch_ids ? batch_ids[num_batch_ids - 1]
						     : 0);
...
int mgmt_be_send_subscr_req(struct mgmt_be_client *client_ctx,
			    bool subscr_xpaths, int num_xpaths, char **reg_xpaths)
{
	Mgmtd__BeMessage be_msg;
...
static int _notify_conenct_disconnect(struct msg_client *msg_client, bool connected)
{
	struct mgmt_be_client *client =
