...
/* ------------------------------------------------- */
/* weak reference symbols to be implemented in mgmtd */
/* ------------------------------------------------- */

extern void vty_new_mgmt(struct vty *new);

/* config enter/exit calls */
extern int vty_config_enter_mgmt(struct vty *vty, bool private_config, bool exclusive,
				 bool file_lock);
extern void vty_config_node_exit_mgmt(struct vty *vty);

/* XFRR_start/_end calls around config file processing */
extern void vty_start_config_mgmt(struct vty *vty);
extern void vty_end_config_mgmt(struct vty *vty);

/* process CLI apply-changes */
extern int nb_cli_apply_changes_mgmt(struct vty *vty, const char *xpath_base_abs);

/* process RPC */
extern int nb_cli_rpc_mgmt(struct vty *vty, const char *xpath, const struct lyd_node *input);
