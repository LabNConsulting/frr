...
{
	bool have_apptag = ei->apptag[0] != 0;
	bool have_path = ei->path[0] != 0;
	bool have_msg = ei->msg[0] != 0;
...
int vty_mgmt_send_get_tree_req(struct vty *vty, bool is_config,
			       Mgmtd__DatastoreId datastore, LYD_FORMAT result_type,
			       const char *xpath)
{
	/* we support working in LYD_LYB, LYD_JSON, LYD_XML */
	LYD_FORMAT int_format = LYD_LYB;

	vty->mgmt_req_id++;

	if (mgmt_fe_send_get_tree_req(mgmt_fe_client, vty->mgmt_session_id,
				      vty->mgmt_req_id, datastore, is_config,
				      int_format, xpath)) {
