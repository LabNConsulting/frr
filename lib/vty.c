...
__attribute__((weak)) int vty_config_enter_mgmt(struct vty *vty, bool private_config,
						bool exclusive, bool file_lock)
{
	return CMD_SUCCESS;
}

__attribute__((weak)) void vty_config_node_exit_mgmt(struct vty *vty)
{
}

__attribute__((weak)) void vty_start_config_mgmt(struct vty *vty)
{
}

__attribute__((weak)) void vty_end_config_mgmt(struct vty *vty)
{
}

__attribute__((weak)) void vty_new_mgmt(struct vty *new)
{
	return;
}

__attribute__((weak)) int nb_cli_apply_changes_mgmt(struct vty *vty, const char *xpath_base_abs)
{
	return CMD_SUCCESS;
}

__attribute__((weak)) int nb_cli_rpc_mgmt(struct vty *vty, const char *xpath,
		     ...
		     bool file_lock)
{
	...
	if (exclusive && !vty_mgmt_fe_enabled() && nb_running_lock(NB_CLIENT_CLI, vty)) {
		vty_out(vty, "%% Configuration is locked by other client\n");
		return CMD_WARNING;
	}

	ret = vty_config_enter_mgmt(vty, private_config, exclusive, file_lock);
	if (ret != CMD_SUCCESS)
		return ret;
