 ...
 * @return	1 if equal, 0 otherwise
 */
extern int ls_attributes_same(struct ls_attributes *a1, struct ls_attributes *a2);
...
macro_inline int edge_cmp(const struct ls_edge *edge1, const struct ls_edge *edge2)
{
 ...
 */
extern struct ls_vertex *ls_vertex_update(struct ls_ted *ted, struct ls_node *node);
...
ls_find_edge_by_destination(struct ls_ted *ted, struct ls_attributes *attributes);
 ...
 * @return	New Subnet
 */
extern struct ls_subnet *ls_subnet_add(struct ls_ted *ted, struct ls_prefix *pref);
 ...
 */
extern void ls_connect(struct ls_vertex *vertex, struct ls_edge *edge, bool source);
 ...
 *			to the provided Link State Message
 */
extern struct ls_message *ls_edge2msg(struct ls_message *msg, struct ls_edge *edge);
