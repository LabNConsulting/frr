...
nb_apply_finish_cb_find(struct nb_config_cbs *cbs, const struct nb_node *nb_node,
			const struct lyd_node *dnode)
{
			 ...
			 uint32_t flags, nb_oper_data_cb cb, void *arg)
{
	...
	for (ALL_LIST_ELEMENTS_RO(list_dnodes, ln, dn)) {
		...
		list_entry = nb_callback_lookup_entry(nn, list_entry,
						      &list_keys);
		if (list_entry == NULL) {
			list_delete(&list_dnodes);
			yang_dnode_free(dnode);
...
void nb_running_move_tree(const char *xpath_from, const char *xpath_to)
{
	...
	for (ALL_LIST_ELEMENTS_RO(entries, ln, entry)) {
		...
		char *newpath = frrstr_replace(entry->xpath, xpath_from,
					       xpath_to);
