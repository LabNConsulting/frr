...
void _install_element(enum node_type ntype, const struct cmd_element *cmd)
{
	...
	if (cnode->graph_built || !defer_cli_tree) {
		...
		struct cmd_token *token = cmd_token_new(START_TKN, 0, NULL,
							NULL);
			...
			const struct cmd_element **cmd, int vtysh)
{
	...
	saved_ret = ret = cmd_execute_command_real(vline, FILTER_RELAXED, vty,
						   cmd, 0);
...
#define O_DIRECTORY 0
	...
	size_t config_file_sav_sz = strlen(config_file) +
      ...
      "The password string\n")
{
	...
	if (host.encrypt) {
		if (host.password_encrypt)
			XFREE(MTYPE_HOST, host.password_encrypt);
		host.password_encrypt = XSTRDUP(MTYPE_HOST,
						zencrypt(argv[idx_8]->arg));
	} else
      ...
      "The HIDDEN 'enable' password string\n")
{
	...
	if (argc == 4) {
		if (argv[idx_8]->arg[0] == '8') {
			...
			host.enable_encrypt = XSTRDUP(MTYPE_HOST,
						      argv[idx_word]->arg);
