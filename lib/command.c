 ...
 */
static vector cmd_complete_command_real(vector vline, struct vty *vty, int *status)
{
...
void cmd_variable_complete(struct cmd_token *token, const char *arg, vector comps)
{
	struct listnode *ln;
 ...
 */
static int handle_pipe_action(struct vty *vty, const char *cmd_in, char **cmd_out)
{
	/* look for `|` */
				 ...
				 uint32_t line_num, int use_daemon)
{
	...
	while (!(use_daemon && ret == CMD_SUCCESS_DAEMON) &&
	       !(!use_daemon && ret == CMD_ERR_NOTHING_TODO) && ret != CMD_SUCCESS &&
	       ret != CMD_WARNING && ret != CMD_ERR_AMBIGUOUS &&
	       ret != CMD_ERR_INCOMPLETE && ret != CMD_NOT_MY_INSTANCE &&
	       ret != CMD_WARNING_CONFIG_FAILED && ret != CMD_NO_LEVEL_UP)
...
DEFUN(config_no_domainname, no_domainname_cmd, "no domainname [DOMAINNAME]",
      NO_STR "Reset system's domain name\n"
...
DEFUN(config_enable_password, enable_password_cmd, "enable password [(8-8)] WORD",
