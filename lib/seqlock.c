...
#define wait_time(sqlo, val, time, reltime)                                    \
	sys_futex((int *)&sqlo->pos, FUTEX_WAIT_BITSET, (int)val, time, NULL,  \
		  ~0U)
...
void seqlock_wait(struct seqlock *sqlo, seqlock_val_t val)
{
	...
	while (cur & SEQLOCK_HELD) {
		...
		if ((cur & SEQLOCK_WAITERS) ||
		    atomic_compare_exchange_weak_explicit(&sqlo->pos, &cur,
							  cur | SEQLOCK_WAITERS,
	...
	while (cur & SEQLOCK_HELD) {
		...
		if ((cur & SEQLOCK_WAITERS) ||
			...
			time_prep
				...
				rv = wait_time(sqlo, cur | SEQLOCK_WAITERS,
					       time_arg1, time_arg2);
			if (rv) {
				ret = false;
				break;
...
{
	return SEQLOCK_VAL(
		atomic_load_explicit(&sqlo->pos, memory_order_relaxed));
}
