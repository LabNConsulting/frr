 ...
 */
int mgmt_fe_send_get_tree_req(struct mgmt_fe_client *client, uint64_t session_id,
			      uint64_t req_id, Mgmtd__DatastoreId ds_id,
			      bool is_config, LYD_FORMAT result_type, const char *xpath)
{
	...
	msg = XMALLOC(MTYPE_TMP, mlen);
	msg->session_id = session_id;
	msg->req_id = req_id;
	msg->code = MGMT_MSG_CODE_GET_TREE;
	msg->ds_id = ds_id;
	msg->is_config = is_config;
	msg->result_type = result_type;
	strlcpy(msg->xpath, xpath, xplen + 1);

	MGMTD_FE_CLIENT_DBG("Sending GET_TREE_REQ is-config %d DS:%s session-id %" PRIu64
			    " req-id %" PRIu64 " xpath: %s",
			    is_config, dsid2name(ds_id), session_id, req_id,
			    xpath);
