...
def test_ospf_dscp_all_and_low_control(tgen):
    ...
    assert neighbors_full("r1") or _wait_for_ospf_full(
        "r1"
...
"""
    ...
    topotest.sleep(2, "Gathering Packets")
    ...
    for dscp, msg in tuples:
        ...
        if msg == 1 and dscp == 48:
            hello_ok = True
        if msg in (2, 3, 4) and dscp == 40:
            low_ctrl_ok = True

    assert hello_ok, "No Hello packet from R1 with DSCP 48 observed"
