...
def test_regression_disconnect_after_abort(tgen, stepf):
    r1g = tgen.gears["r1"]
    r1 = r1g.net

    stepf("Config an active VRF")
    output = r1g.vtysh_multicmd(
        """
        show vrf
        conf t
    ...
    stepf('Try to un-config an active vrf (i.e., "no vrf"), verify failure')
    output = r1g.vtysh_multicmd(
        """
        show vrf
        conf t
    ...
    stepf("Check that no clients disconnected due to config validation failure")
    ...
    stepf("Verify TXN_REQ delete sent to all subscribed clients")
    ...
    stepf("Checking for still locked regression")
    check_locked(r1g)
