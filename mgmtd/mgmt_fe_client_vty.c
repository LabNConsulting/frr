...
/* static */ extern struct mgmt_fe_client *mgmt_fe_client;
...
/* ------------------------------------------------------ */
/* Read configuration files for all mgmt-enabled daemons. */
/* ------------------------------------------------------ */
...
/* =============================== */
/* Mgmtd Frontend Client Callbacks */
/* =============================== */
...
static void vty_mgmt_handle_commit_config_reply(struct mgmt_fe_client *client, uintptr_t usr_data,
					    uint64_t client_id, uintptr_t session_id,
					    uintptr_t session_ctx, uint64_t req_id, bool success,
...
/* =============================== */
/* VTY Functionality Hook Funtions */
/* =============================== */

void vty_new_mgmt(struct vty *new)
{
	if (mgmt_fe_client) {
...
int vty_config_enter_mgmt(struct vty *vty, bool private_config, bool exclusive, bool file_lock)
{
	/*
...
void vty_config_node_exit_mgmt(struct vty *vty)
{
	if (vty->mgmt_locked_running_ds)
		vty_mgmt_unlock_running_inline(vty);
...
void vty_start_config_mgmt(struct vty *vty)
{
}

void vty_end_config_mgmt(struct vty *vty)
{
	/*
...
int nb_cli_apply_changes_mgmt(struct vty *vty, const char *xpath_base_abs)
{
...
int nb_cli_rpc_mgmt(struct vty *vty, const char *xpath, const struct lyd_node *input)
{
	char *data = NULL;
	LY_ERR err;
	int ret;
