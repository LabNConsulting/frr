...
static void mgmt_fe_rebuild_ns_strings(void)
{
	struct mgmt_fe_client_adapter *adapter;
	struct mgmt_fe_session_ctx *session;
	struct ns_string_head new;
	const char **path;
	size_t plen;
	...
	FOREACH_ADAPTER_IN_LIST (adapter) {
		FOREACH_SESSION_IN_LIST (adapter, session) {
			darr_foreach_p (session->notify_xpaths, path) {
				plen = strlen(*path);
				mgmt_fe_add_ns_string(&new, *path, plen, session);
			}
		}

		mgmt_fe_free_ns_strings(&mgmt_fe_ns_strings);
		mgmt_fe_ns_strings = new;
	}
}
