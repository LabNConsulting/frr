...
static void fe_adapter_handle_edit(struct mgmt_fe_session_ctx *session, void *_msg, size_t msg_len)
{
	...
	if (ret && ret == NB_ERR_NO_CHANGES)
		...
		_dbg("Edit failed for txn-id %Lu req-id %Lu: %s: restoring candidate",
		...
		mgmt_txn_send_commit_config_req(txn_id, msg->req_id, can_id, can_ds, run_id,
						run_ds, false, false, true /* implicit */,
						false, edit);
		return;
	}
reply:
	mgmt_fe_adapter_send_edit_reply(session->session_id, txn_id, msg->req_id, false, commit, &edit,
				  ...
				  bool unlock, enum mgmt_result result, const char *error_if_any)
{
	...
	if (!session) {
		/* XXXchopps is this the right error type? */
		return -ENOENT;
	}
	...
	if (result == MGMTD_SUCCESS || result == MGMTD_NO_CFG_CHANGES)
		...
		ret = fe_adapter_send_error(
			...
			error_if_any ?: "");
		/* XXXchopps do anything with this error? */
	}
