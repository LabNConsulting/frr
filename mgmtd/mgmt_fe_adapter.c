...
{
	struct mgmt_fe_client_adapter *adapter = session->adapter;
	struct mgmt_ds_ctx *ds_ctx;
	struct mgmt_msg_get_tree *msg = data;
	Mgmtd__DatastoreId ds_id = msg->ds_id;
	uint64_t req_id = msg->req_id;
	uint64_t clients;
	bool is_cfg = msg->is_config;
	int ret;
	...
	if (is_cfg) {
		...
		fe_adapter_send_error(session, req_id, false, -EINVAL,
				      "get-tree msg to adapter %s for config unsupported",
				      adapter->name);
		return;
	}

	if (ds_id != MGMTD_DS_OPERATIONAL) {
		fe_adapter_send_error(session, req_id, false, -EINVAL,
				      "get-req on unsupported datastore");
		return;
	}
