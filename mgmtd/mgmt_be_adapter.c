...
static const char **be_client_xpaths[] = {
#ifdef HAVE_STATICD
				    ...
				    enum mgmt_be_client_id id, bool config)
{
	...
	if (xpaths) {
		for (; *xpaths; xpaths++) {
			if (mgmt_be_eval_regexp_match(*xpaths, xpath)) {
...
static void be_show_xpath_register(struct vty *vty, struct mgmt_be_xpath_map *map)
{
	enum mgmt_be_client_id id;
	const char *astr;
