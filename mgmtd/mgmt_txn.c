				...
				const char *xpath)
{
	...
	msg->ds_id = MGMTD_DS_OPERATIONAL;
				    ...
				    size_t result_len, int partial_error)
{
	...
	/* store the result */
	err = lyd_parse_data_mem(ly_native_ctx, result, result_type,
				 LYD_PARSE_OPAQ | LYD_PARSE_ONLY,
				 0 /*LYD_VALIDATE_OPERATIONAL*/, &tree);
	if (err) {
		MGMTD_TXN_ERR("GETTREE reply from %s for txn-id %" PRIu64
			      " req_id %" PRIu64
			      " error parsing result of result type %u",
			      adapter->name, txn_id, req_id, result_type);
	}
	if (!err) {
