...
static void mgmt_txn_unlock(struct mgmt_txn_ctx **txn, const char *file, int line);
...
static int mgmt_move_be_commit_to_next_phase(struct mgmt_txn_ctx *txn,
					     struct mgmt_be_client_adapter *adapter);
			 ...
			 struct mgmt_be_client_adapter *be_adapter)
{
	...
	mgmt_txn_batches_add_tail(&txn->commit_cfg_req->req.commit_cfg.curr_batches[id],
				  cfg_btch);
	...
	txn->commit_cfg_req->req.commit_cfg.last_be_cfg_batch[id] = cfg_btch;
...
static void mgmt_txn_cfg_batch_free(struct mgmt_txn_be_cfg_batch **cfg_btch)
{
	size_t indx;
	...
	assert((*cfg_btch)->txn && (*cfg_btch)->txn->type == MGMTD_TXN_TYPE_CONFIG);
...
static void mgmt_txn_process_set_cfg(struct event *thread)
{
	...
	FOREACH_TXN_REQ_IN_LIST (&txn->set_cfg_reqs, txn_req) {
		...
		} else if (mgmt_fe_send_set_cfg_reply(txn->session_id, txn->txn_id,
						      txn_req->req.set_cfg->ds_id,
...
static int mgmt_try_move_commit_to_next_phase(struct mgmt_txn_ctx *txn,
					      struct mgmt_commit_cfg_req *cmtcfg_req)
{
	...
	RB_FOREACH_SAFE (cb, nb_config_cbs, changes, nxt) {
		...
		FOREACH_MGMTD_BE_CLIENT_ID (id) {
			...
			if (!cfg_btch ||
				...
				cfg_btch = mgmt_txn_cfg_batch_alloc(txn_req->txn,
								    id, adapter);
			}
			...
			mgmt_yang_data_init(&cfg_btch->data[cfg_btch->num_cfg_data]);
			...
			cfg_btch->value[cfg_btch->num_cfg_data].encoded_str_val =
				value;
			value = NULL;
...
static int mgmt_txn_prepare_config(struct mgmt_txn_ctx *txn)
{
	...
	if (mm->perf_stats_en)
		gettimeofday(&txn->commit_cfg_req->req.commit_cfg.cmt_stats->validate_start,
			     NULL);
	/*
...
mgmt_txn_prep_config_validation_done:
	...
	if (mm->perf_stats_en)
		gettimeofday(&txn->commit_cfg_req->req.commit_cfg.cmt_stats->prep_cfg_start,
			     NULL);
	...
	FOREACH_MGMTD_BE_CLIENT_ID (id) {
		if (cmtcfg_req->subscr_info.xpath_subscr[id]) {
			...
			FOREACH_TXN_CFG_BATCH_IN_LIST (
				...
				cfg_btch)
				cfg_btch->comm_phase = MGMTD_COMMIT_PHASE_TXN_CREATE;
		}
	}
				     ...
				     struct mgmt_be_client_adapter *adapter)
{
	...
	FOREACH_TXN_CFG_BATCH_IN_LIST (&cmtcfg_req->curr_batches[adapter->id],
		...
		indx++;
		if (mgmt_be_send_cfgdata_req(adapter, txn->txn_id, cfg_btch->batch_id,
			...
			MGMTD_TXN_ERR("Could not send CFGDATA_CREATE txn-id: %" PRIu64
				      ...
				      txn->txn_id, cfg_btch->batch_id,
				      adapter->name);
			return -1;
		}

		cmtcfg_req->cmt_stats->last_num_cfgdata_reqs++;
		mgmt_move_txn_cfg_batch_to_next(cmtcfg_req, cfg_btch,
...
static void mgmt_txn_cfg_commit_timedout(struct event *thread)
{
	 ...
	 * adapter.
	 */
	mgmt_txn_send_commit_cfg_reply(txn, MGMTD_INTERNAL_ERROR,
	...
	FOREACH_MGMTD_BE_CLIENT_ID (id) {
		if (cmtcfg_req->subscr_info.xpath_subscr[id] &
			...
			FOREACH_TXN_CFG_BATCH_IN_LIST (btch_list, cfg_btch)
				cfg_btch->comm_phase = MGMTD_COMMIT_PHASE_APPLY_CFG;
		}
	}
...
static int mgmt_txn_get_config(struct mgmt_txn_ctx *txn,
			       struct mgmt_txn_req *txn_req, struct nb_config *root)
{
	int indx;
	struct mgmt_get_data_req *get_data;
	struct mgmt_get_data_reply *get_reply;
...
static struct mgmt_txn_ctx *mgmt_fe_find_txn_by_session_id(struct mgmt_master *cm,
							   uint64_t session_id,
							   enum mgmt_txn_type type)
{
	struct mgmt_txn_ctx *txn;
				 ...
				 Mgmtd__YangGetDataReq **data_req, size_t num_reqs)
{
	...
	for (indx = 0;
	     indx < num_reqs && indx < MGMTD_MAX_NUM_DATA_REPLY_IN_BATCH; indx++) {
