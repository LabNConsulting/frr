		...
		default:
			if (IS_ZEBRA_DEBUG_PACKET)
				zlog_debug("%s:Received Packet with optional Header type %s(%u) that is being ignored",
					   __func__,
					   rtadv_optionalhdr2str(opt_hdr->nd_opt_type),
					   opt_hdr->nd_opt_type);
			break;
		}
...
#define SIXHOUR2USEC (int64_t)6 * 60 * 60 * 1000000
	...
	if ((radvert->nd_ra_flags_reserved & ND_RA_FLAG_OTHER) &&
	    ...
	    (monotime_since(&zif->rtadv.lastadvotherconfigflag, NULL) > SIXHOUR2USEC ||
	...
	if ((radvert->nd_ra_reachable && zif->rtadv.AdvReachableTime) &&
	    ...
	    (monotime_since(&zif->rtadv.lastadvreachabletime, NULL) > SIXHOUR2USEC ||
...
static void rtadv_process_packet(uint8_t *buf, unsigned int len,
				 ifindex_t ifindex, int hoplimit,
				 struct sockaddr_in6 *from, struct zebra_vrf *zvrf)
{
...
DEFUN(no_ipv6_nd_adv_interval_config_option,
      no_ipv6_nd_adv_interval_config_option_cmd, "no ipv6 nd adv-interval-option",
      NO_STR "Interface IPv6 config commands\n"
	     "Neighbor discovery\n"
	     "Advertisement Interval Option\n")
{
