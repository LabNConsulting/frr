...
void zebra_evpn_print(struct zebra_evpn *zevpn, void **ctxt)
{
	...
	if (!zevpn->vteps) {
		...
		for (zvtep = zevpn->vteps; zvtep; zvtep = zvtep->next) {
			const char *flood_str =
				lookup_msg(zvtep_flood_str, zvtep->flood_control,
...
static int ip_prefix_send_to_client(vrf_id_t vrf_id, struct prefix *p, uint16_t cmd)
{
			    ...
			    struct ipaddr *ip)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("%u:SVI %s(%u) VNI %u, sending GW MAC %pEA IP %pIA del to BGP",
			   ifp->vrf->vrf_id, ifp->name, ifp->ifindex,
			   zevpn->vni, &n->emac, ip);
...
void zebra_evpn_gw_macip_add_for_evpn_hash(struct hash_bucket *bucket, void *ctxt)
{
	...
	if (advertise_svi_macip_enabled(zevpn) || advertise_gw_macip_enabled(zevpn))
...
static int zebra_evpn_map_vlan_ns(struct ns *ns, void *_in_param, void **_p_zevpn)
{
	int found = 0;
...
static int zebra_evpn_from_svi_ns(struct ns *ns, void *_in_param, void **_p_zevpn)
{
	struct zebra_ns *zns = ns->info;
	struct route_node *rn;
	struct interface *br_if;
 ...
 */
struct zebra_evpn *zebra_evpn_from_svi(struct interface *ifp, struct interface *br_if)
{
	struct zebra_evpn *zevpn = NULL;
	struct zebra_evpn **p_zevpn;
	struct zebra_if *zif;
 ...
 */
static void zebra_evpn_uninstall_mac_hash(struct hash_bucket *bucket, void *ctxt)
{
...
void zebra_evpn_read_mac_neigh(struct zebra_evpn *zevpn, struct interface *ifp)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("Reading MAC FDB and Neighbors for intf %s(%u) VNI %u master %u",
 ...
 */
static int zebra_evpn_vtep_match(struct in_addr *vtep_ip, struct zebra_vtep *zvtep)
{
 ...
 */
void zebra_evpn_handle_flooding_remote_vteps(struct hash_bucket *bucket, void *zvrf)
{
	struct zebra_evpn *zevpn;
	struct zebra_vtep *zvtep;
					      ...
					      const esi_t *esi)
{
	...
	if (!ipa_len) {
		...
		if (n &&
		    !zebra_evpn_neigh_is_bgp_seq_ok(zevpn, n, macaddr, seq, true))
			return;
			      ...
			      struct in_addr vtep_ip, const esi_t *esi)
{
	...
	if (!ifp || !if_is_operative(ifp) || !zif || !zif->brslave_info.br_if) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("Ignoring remote MACIP ADD VNI %u, invalid interface state or info",
				   vni);
		return;
	}
			      ...
			      struct in_addr vtep_ip)
{
	...
	if (!vnip) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("VNI %u not in interface upon remote MACIP DEL",
				   vni);
		return;
	}
	...
	if (!mac && !n) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("Failed to locate MAC %pEA & Neigh %pIA VNI %u upon remote MACIP DEL",
				   macaddr, ipaddr, vni);
		return;
	}
