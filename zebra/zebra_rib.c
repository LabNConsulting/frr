...
static void rib_process_result(struct zebra_dplane_ctx *ctx)
{
	...
	case DPLANE_OP_ROUTE_UPDATE:
		if (status == ZEBRA_DPLANE_REQUEST_SUCCESS) {
			 ...
			 */
			if (re) {
				fib_changed = rib_update_re_from_ctx(re, rn,
								     ctx);
	...
	case RIB_TABLES_ITER_S_ITERATING:
		...
		while (1) {
			while (iter->afi_safi_ix < (int)array_size(afi_safis)) {
				table = zebra_vrf_table(afi_safis[iter->afi_safi_ix]
								.afi,
							afi_safis[iter->afi_safi_ix]
								.safi,
							iter->vrf_id);
				if (table)
					break;
