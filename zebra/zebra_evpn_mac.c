 ...
 * bridge port member
 */
static void zebra_evpn_mac_ifp_link(struct zebra_mac *zmac, struct interface *ifp)
{
	struct zebra_if *zif;
					     ...
					     struct zebra_mac *mac)
{
	if ((wctx->flags & DEL_LOCAL_MAC) && (mac->flags & ZEBRA_MAC_LOCAL))
		...
		return true;
		 ...
		 (mac->flags & ZEBRA_MAC_AUTO) && !listcount(mac->neigh_list)) {
		if (IS_ZEBRA_DEBUG_VXLAN) {
			char mac_buf[MAC_BUF_SIZE];
...
int zebra_evpn_add_update_local_mac(struct zebra_vrf *zvrf,
				    struct zebra_evpn *zevpn, struct interface *ifp,
	...
	if (!mac) {
		...
		if (CHECK_FLAG(mac->flags, ZEBRA_MAC_LOCAL)) {
			...
			if (mac_sticky == sticky && old_ifp == ifp &&
			    old_vid == vid && old_local_inactive == local_inactive &&
...
void zebra_evpn_mac_svi_add(struct interface *ifp, struct zebra_evpn *zevpn)
{
	...
	if (!zebra_evpn_mh_do_adv_svi_mac() || !zebra_evpn_send_to_client_ok(zevpn))
		return;
