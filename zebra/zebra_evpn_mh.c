...
static void zebra_evpn_acc_vl_show_entry_detail(
	struct vty *vty, struct zebra_evpn_access_bd *acc_bd, json_object *json)
{
	struct zebra_if *zif;
	struct listnode *node;
...
void zebra_evpn_es_set_base_evpn(struct zebra_evpn *zevpn)
{
	...
	for (ALL_LIST_ELEMENTS_RO(zmh_info->local_es_list, node, es)) {
		...
		if (es->flags & ZEBRA_EVPNES_READY_FOR_BGP)
			...
			zebra_evpn_es_re_eval_send_to_client(
				es, true /* es_evi_re_reval */);
	}
}
