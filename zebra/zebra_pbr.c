...
static void zebra_pbr_expand_rule(struct zebra_pbr_rule *rule)
{
	...
	table = zebra_vrf_get_table_with_table_id(AFI_IP, SAFI_UNICAST,
						  VRF_DEFAULT,
						  rule->rule.action.table);
	if (!table)
		return;
...
static const char *zebra_pbr_prefix2str(union prefixconstptr pu, char *str,
					int size)
{
				   ...
				   uint8_t proto)
{
	if (!(filter_bm & PBR_FILTER_PROTO)) {
		...
		if (port_max)
			vty_out(vty, ":proto %d:%d-%d", proto, port_min,
				port_max);
		else
...
size_t zebra_pbr_tcpflags_snprintf(char *buffer, size_t len, uint16_t tcp_val)
{
	...
	for (pnt = tcp_value_str; memcmp(pnt, &nt, sizeof(struct message));
	     pnt++)
		if (pnt->key & tcp_val) {
	...
	if (iptable->fragment) {
		...
		vty_out(vty, "\t fragment%s %s\n",
			iptable->filter_bm & MATCH_FRAGMENT_INVERSE_SET ? " not"
									: "",
			lookup_msg(fragment_value_str, iptable->fragment,
				   val_str));
	}
	if (iptable->protocol) {
