...
static void zebra_mlag_client_msg_handler(struct event *event)
{
	...
	for (wr_count = 0; wr_count < max_count; wr_count++) {
		 ...
		 */
		if (len > 0) {
			hook_call(zebra_mlag_private_write_data, mlag_wr_buffer,
				  len);
...
static void zebra_mlag_post_data_from_main_thread(struct event *thread)
{
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("%s: Posting MLAG data for msg_type:0x%x to interested clients",
			   __func__, msg_type);
...
void zebra_mlag_client_register(ZAPI_HANDLER_ARGS)
{
	...
	if (client->mlag_updates_interested == true) {
		if (IS_ZEBRA_DEBUG_MLAG)
			zlog_debug("Client is registered, existing mask: 0x%x, new mask: 0x%x",
	...
	if (zrouter.mlag_info.clients_interested_cnt == 1) {
		...
		if (rc < 0) {
			 ...
			 */
			if (IS_ZEBRA_DEBUG_MLAG)
				zlog_debug("Fail to open channel with MLAG,rc:%d, post Proto-down",
					   rc);
			zebra_mlag_publish_process_state(client,
...
void zebra_mlag_forward_client_msg(ZAPI_HANDLER_ARGS)
{
	 ...
	 */
	stream_put(mlag_s, zebra_s->data + zebra_s->getp, STREAM_READABLE(zebra_s));
...
int zebra_mlag_protobuf_encode_client_data(struct stream *s, uint32_t *msg_type)
{
	...
	*msg_type = mlag_msg.msg_type;
	...
	if (IS_ZEBRA_DEBUG_MLAG)
		zlog_debug("%s: length of Mlag ProtoBuf message:%s with Header  %d",
			   __func__,
			   mlag_lib_msgid_to_str(mlag_msg.msg_type, buf,
						 sizeof(buf)),
			   len);
...
int zebra_mlag_protobuf_decode_message(struct stream *s, uint8_t *data, uint32_t len)
{
	uint32_t msg_type;
	ZebraMlagHeader *hdr;
	char buf[80];
	...
	if (hdr->data.len == 0) {
		...
		case ZEBRA_MLAG__HEADER__MESSAGE_TYPE__ZEBRA_MLAG_MROUTE_ADD_BULK: {
			...
			Bulk_msg =
				zebra_mlag_mroute_add_bulk__unpack(NULL,
								   hdr->data.len,
								   hdr->data.data);
			if (Bulk_msg == NULL) {
				zebra_mlag__header__free_unpacked(hdr, NULL);
			...
			for (i = 0; i < Bulk_msg->n_mroute_add; i++) {
				if (STREAM_SIZE(s) <
					...
					zlog_warn("We have received more messages than we can parse at this point in time: %zu",
		...
		case ZEBRA_MLAG__HEADER__MESSAGE_TYPE__ZEBRA_MLAG_MROUTE_DEL_BULK: {
			...
			Bulk_msg =
				zebra_mlag_mroute_del_bulk__unpack(NULL,
								   hdr->data.len,
								   hdr->data.data);
			if (Bulk_msg == NULL) {
				zebra_mlag__header__free_unpacked(hdr, NULL);
			...
			for (i = 0; i < Bulk_msg->n_mroute_del; i++) {
				if (STREAM_SIZE(s) <
					...
					zlog_warn("We have received more messages than we can parse at this time");
					break;
				}
		...
		case ZEBRA_MLAG__HEADER__MESSAGE_TYPE__ZEBRA_MLAG_ZEBRA_STATUS_UPDATE: {
			...
			msg = zebra_mlag_zebra_status_update__unpack(NULL,
								     hdr->data.len,
								     hdr->data.data);
			if (msg == NULL) {
				zebra_mlag__header__free_unpacked(hdr, NULL);
