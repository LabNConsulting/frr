...
static int zl3vni_from_svi_ns(struct ns *ns, void *_in_param, void **_p_zl3vni)
{
	...
	if (in_param->bridge_vlan_aware) {
		...
		for (rn = route_top(zns->if_table); rn; rn = route_next(rn)) {
			...
			vni_id =
				zebra_vxlan_if_access_vlan_vni_find(zif,
								    in_param->br_if);
			if (vni_id) {
				found = 1;
				break;
...
void zebra_vxlan_remote_macip_del(ZAPI_HANDLER_ARGS)
{
	...
	while (l < hdr->length) {
		int res_length = zebra_vxlan_remote_macip_helper(false, s, &vni,
								 &macaddr,
								 &ipa_len, &ip,
								 &vtep_ip, NULL,
								 NULL, NULL);
				    ...
				    int err_str_sz, int filter, int add)
{
	...
	if (add) {
		...
		if (zvrf->l3vni != vni) {
			snprintf(err, err_str_sz,
				 "VNI %d doesn't exist in VRF: %s", vni,
				 zvrf->vrf->name);
			return -1;
		}
...
static unsigned int zebra_vxlan_sg_hash_key_make(const void *p)
{
	...
	return (jhash_2words(vxlan_sg->sg.src.s_addr, vxlan_sg->sg.grp.s_addr,
			     0));
}
