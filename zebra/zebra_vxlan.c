...
static int zl3vni_nh_uninstall(struct zebra_l3vni *zl3vni, struct zebra_neigh *n);
...
static int zl3vni_rmac_install(struct zebra_l3vni *zl3vni, struct zebra_mac *zrmac);
...
static void zebra_vxlan_sg_do_deref(struct zebra_vrf *zvrf, struct in_addr sip,
 ...
 */
static void zevpn_print_neigh_hash_all_evpn(struct hash_bucket *bucket, void **args)
{
	struct vty *vty;
...
static void zl3vni_print_nh_hash_all_vni(struct hash_bucket *bucket, void **args)
{
	struct vty *vty = NULL;
...
static void zl3vni_print_rmac_hash_all_vni(struct hash_bucket *bucket, void **args)
{
	struct vty *vty = NULL;
	json_object *json = NULL;
	json_object *json_evpn = NULL;
...
static int zvni_map_to_svi_ns(struct ns *ns, void *_in_param, void **_p_ifp)
{
				      ...
				      struct zebra_vxlan_vni *vnip, void *arg)
{
	...
	if (zl3vni) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("Create L2-VNI hash for intf %s(%u) L2-VNI %u local IP %pI4",
 ...
 */
static int zl3vni_rmac_uninstall(struct zebra_l3vni *zl3vni, struct zebra_mac *zrmac)
{
				   ...
				   struct zebra_mac *zrmac, struct ipaddr *vtep_ip)
{
	...
	if (!zl3vni_nh_lookup(zl3vni, vtep_ip)) {
		...
		if (!listcount(zrmac->nh_list)) {
			...
			if (IS_ZEBRA_DEBUG_VXLAN)
				zlog_debug("L3VNI %u RMAC %pEA vtep_ip %pIA delete",
		       ...
		       struct zebra_neigh *n)
{
	...
	if (!(n->flags & ZEBRA_NEIGH_REMOTE) || !(n->flags & ZEBRA_NEIGH_REMOTE_NH))
		return 0;
...
static void zl3vni_remote_nh_del(struct zebra_l3vni *zl3vni,
				 struct zebra_neigh *nh, struct prefix *host_prefix)
{
			     ...
			     const struct prefix *host_prefix)
{
	...
	if (!nh) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("SVD RMAC change(%pEA --> %pEA) for nexthop %pIA, prefix %pFX",
...
static int svd_remote_nh_del(struct zebra_l3vni *zl3vni, const struct ipaddr *vtep_ip)
{
...
static int zl3vni_map_to_vxlan_if_ns(struct ns *ns, void *_zl3vni, void **_pifp)
{
	struct zebra_ns *zns = ns->info;
...
static int zl3vni_from_svi_ns(struct ns *ns, void *_in_param, void **_p_zl3vni)
{
	...
	if (in_param->bridge_vlan_aware) {
		...
		for (rn = route_top(zns->if_table); rn; rn = route_next(rn)) {
			...
			vni_id = zebra_vxlan_if_access_vlan_vni_find(zif,
								     in_param->br_if);
			if (vni_id) {
				found = 1;
				break;
					     ...
					     int add)
{
	...
	if (add) {
		...
		if (!found) {
			if (IS_ZEBRA_DEBUG_VXLAN)
				zlog_err("Adding L2-VNI - Failed to find VxLAN interface for VNI %u",
					 vni);
			return -1;
		}
					 ...
					 struct ethaddr *rmac)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("Del remote RMAC %pEA L3VNI %u - readd", rmac,
			   zl3vni->vni);
...
void zebra_vxlan_evpn_vrf_route_del(vrf_id_t vrf_id, struct ipaddr *vtep_ip,
 ...
 */
void zebra_vxlan_print_neigh_all_vni_detail(struct vty *vty, struct zebra_vrf *zvrf,
					    bool print_dup, bool use_json)
{
	json_object *json = NULL;
	void *args[3];
 ...
 * (VTY command handler).
 */
void zebra_vxlan_print_neigh_vni_dad(struct vty *vty, struct zebra_vrf *zvrf,
				     vni_t vni, bool use_json)
{
	struct zebra_evpn *zevpn;
	uint32_t num_neigh;
	...
	if (!use_json) {
		...
		vty_out(vty, "%*s %-6s %-8s %-17s %-30s\n", -wctx.addr_width,
			"IP", "Type", "State", "MAC", "Remote ES/VTEP");
	} else
		json_object_int_add(json, "numArpNd", num_neigh);
 ...
 */
void zebra_vxlan_print_macs_all_vni_detail(struct vty *vty, struct zebra_vrf *zvrf,
					   bool print_dup, bool use_json)
{
	struct mac_walk_ctx wctx;
	json_object *json = NULL;
...
void zebra_vxlan_print_macs_vni_dad(struct vty *vty, struct zebra_vrf *zvrf,
				    vni_t vni, bool use_json)
{
	struct zebra_evpn *zevpn;
	struct mac_walk_ctx wctx;
 ...
 */
void zebra_vxlan_print_vnis(struct vty *vty, struct zebra_vrf *zvrf, bool use_json)
{
	json_object *json = NULL;
	void *args[2];
...
void zebra_vxlan_dup_addr_detection(ZAPI_HANDLER_ARGS)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("VRF %s duplicate detect %s max_moves %u timeout %u freeze %s freeze_time %u",
					...
					struct ipaddr *ip)
{
	...
	if (!zevpn) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("%s: Del neighbor %pIA EVPN is not present for interface %s",
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("Del neighbor %pIA intf %s(%u) -> L2-VNI %u", ip,
...
void zebra_vxlan_remote_macip_del(ZAPI_HANDLER_ARGS)
{
	...
	while (l < hdr->length) {
		int res_length =
			zebra_vxlan_remote_macip_helper(false, s, &vni, &macaddr,
							&ipa_len, &ip, &vtep_ip,
							NULL, NULL, NULL);
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("Recv MACIP DEL VNI %u MAC %pEA%s%s Remote VTEP %pI4 from %s",
				   ...
				   ipa_len ? ipaddr2str(&ip, buf1, sizeof(buf1))
					   : "",
...
void zebra_vxlan_remote_macip_add(ZAPI_HANDLER_ARGS)
{
	...
	while (l < hdr->length) {
		int res_length =
			zebra_vxlan_remote_macip_helper(true, s, &vni, &macaddr,
							&ipa_len, &ip, &vtep_ip,
							&flags, &seq, &esi);
		...
		if (IS_ZEBRA_DEBUG_VXLAN) {
			...
			zlog_debug("Recv %sMACIP ADD VNI %u MAC %pEA%s%s flags 0x%x seq %u VTEP %pI4 ESI %s from %s",
				   (flags & ZEBRA_MACIP_TYPE_SYNC_PATH) ? "sync-"
									: "",
				   vni, &macaddr, ipa_len ? " IP " : "",
				   ipa_len ? ipaddr2str(&ip, buf1, sizeof(buf1))
					   : "",
 ...
 */
int zebra_vxlan_dp_network_mac_del(struct interface *ifp, struct interface *br_if,
				   struct ethaddr *macaddr, vlanid_t vid, vni_t vni)
{
	struct zebra_if *zif = NULL;
	struct zebra_evpn *zevpn = NULL;
 ...
 */
int zebra_vxlan_local_mac_add_update(struct interface *ifp, struct interface *br_if,
				     struct ethaddr *macaddr, vlanid_t vid,
				     ...
				     bool dp_static)
{
	...
	if (!zevpn) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("        Add/Update %sMAC %pEA intf %s(%u) VID %u, could not find EVPN",
	...
	if (!zevpn->vxlan_if) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("        VNI %u hash %p doesn't have intf upon local MAC ADD",
				   zevpn->vni, zevpn);
		return -1;
	}
...
void zebra_vxlan_remote_vtep_del_zapi(ZAPI_HANDLER_ARGS)
{
	...
	while (l < hdr->length) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("Recv VTEP DEL %pI4 VNI %u from %s", &vtep_ip,
 ...
 * Handle message from client to delete a remote VTEP for an EVPN.
 */
void zebra_vxlan_remote_vtep_del(vrf_id_t vrf_id, vni_t vni, struct in_addr vtep_ip)
{
	struct zebra_evpn *zevpn;
				    ...
				    int err_str_sz, int filter, int add)
{
	...
	if (add) {
		...
		if (zvrf->l3vni != vni) {
			snprintf(err, err_str_sz, "VNI %d doesn't exist in VRF: %s",
				 vni, zvrf->vrf->name);
			return -1;
		}
...
void zebra_vxlan_advertise_gw_macip(ZAPI_HANDLER_ARGS)
{
	...
	if (!vni) {
		...
		if (advertise_gw_macip_enabled(zevpn))
			...
			hash_iterate(zvrf->evpn_table,
				     zebra_evpn_gw_macip_del_for_evpn_hash, NULL);
...
static int macfdb_read_ns(struct ns *ns, void *_in_param __attribute__((unused)),
...
static int neigh_read_ns(struct ns *ns, void *_in_param __attribute__((unused)),
			 void **out_param __attribute__((unused)))
{
	struct zebra_ns *zns = ns->info;
...
static int zebra_vxlan_sg_send(struct zebra_vrf *zvrf, struct prefix_sg *sg,
